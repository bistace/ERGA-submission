...
def get_exp_xml(
    ...
    elements["center_name"] = center
    elements["alias"] = alias
    experiments = get_attributes(
        root["exp"], exp_xml, experiments, "EXPERIMENT", **elements
    )

    attributes = ""
    attributes = get_attributes(
        root["exp"], experiments, attributes, "TITLE", **{exp_title: ""}
    )
    attributes = get_attributes(
        root["exp"], experiments, attributes, "STUDY_REF", **{"refname": study_alias}
    )
    ...
    library = ""
    library = get_attributes(
        root["exp"], design, library, "LIBRARY_NAME", **{lib_name: ""}
    )
    library = get_attributes(
        root["exp"], design, library, "LIBRARY_STRATEGY", **{library_strategy: ""}
    )
    library = get_attributes(
        root["exp"], design, library, "LIBRARY_SOURCE", **{source: ""}
    )
    library = get_attributes(
        root["exp"], design, library, "LIBRARY_SELECTION", **{library_selection: ""}
    )
    layout_lib = ""
    ...
    if add_lib:
        ...
        for dict in my_dict:
            key = dict.split(":")
            library = get_attributes(
                root["exp"], design, library, key[0], **{key[1]: ""}
            )
    ...
    machine = ""
    machine = get_attributes(
        root["exp"], platform, machine, "INSTRUMENT_MODEL", **{instrument: ""}
    )

    if add_exp:
        attributes = get_attributes(
            root["exp"], experiments, attributes, "EXPERIMENT_ATTRIBUTES"
        )
        experiment = ""
        my_dict = add_exp.split(";")
        for dict in my_dict:
            key = dict.split(":")
            experiment = get_attributes(
                root["exp"], attributes, experiment, "EXPERIMENT_ATTRIBUTE"
            )
            TAGS = ""
            TAGS = get_attributes(root["exp"], experiment, TAGS, "TAG", **{key[0]: ""})
            TAGS = get_attributes(
                root["exp"], experiment, TAGS, "VALUE", **{key[1]: ""}
            )
...
def get_runs_xml(center, alias, exp_alias, element, files_reverse):
    ...
    attributes = ""
    attributes = get_attributes(
        root["runs"], runs, attributes, "EXPERIMENT_REF", **{"refname": exp_alias}
    )
    ...
    if element in files_reverse:
        ...
        type = get_attributes(
            root["runs"], file, type, "READ_TYPE", **{"sample_barcode": ""}
        )
        file = get_attributes(
            root["runs"],
            files,
            ...
            "FILE",
            **{
                "filename": files_reverse[element],
                "checksum_method": "MD5",
                "checksum": md5sum[files_reverse[element]],
                "filetype": filetype[element],
            }
        )
        type = get_attributes(root["runs"], file, type, "READ_TYPE", **{"paired": ""})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="all",
        nargs="+",
        choices=["all", "study", "experiment", "runs"],
        help="specify which xml files do you want",
    )
    parser.add_argument(
        "-o", "--out_prefix", required=True, help="prefix to add to output files"
    )
    ...
    for i in in_file.index:
        ...
        if (
            "center" not in in_file
            or pd.isna(in_file["center"][i])
            or in_file["center"][i] == "-"
        ):
            center = "CNAG"
        else:
        ...
        if (
            "scientific_name" not in in_file
            or pd.isna(in_file["scientific_name"][i])
            or in_file["scientific_name"][i] == "-"
        ):
        ...
        if (
            "tolid" not in in_file
            or pd.isna(in_file["tolid"][i])
            or in_file["tolid"][i] == "-"
        ):
        ...
        cname = ""
        if (
            "common_name" in in_file
            and not pd.isna(in_file["common_name"][i])
            and not in_file["common_name"][i] == "-"
        ):
            cname = in_file["common_name"][i]
        elif (
            "common_names" in in_file
            and not pd.isna(in_file["common_names"][i])
            and not in_file["common_names"][i] == "-"
        ):
            cname = in_file["common_names"][i]

        sample_coordinator = ""
        if (
            "sample_coordinator" not in in_file
            or pd.isna(in_file["sample_coordinator"][i])
            or in_file["sample_coordinator"][i] == "-"
        ):
            if args.project == "ERGA-pilot":
                exit(
                    "Missing sample ambassador details, required argument for ERGA-pilot genomes."
                )
        else:
        ...
        locus_tag = "-"
        if (
            "locus_tag" in in_file
            and not pd.isna(in_file["locus_tag"][i])
            and not in_file["locus_tag"][i] == "-"
        ):
            locus_tag = in_file["locus_tag"][i]

        sample_id = ""
        if (
            "sample_tube_or_well_id" in in_file
            and not pd.isna(in_file["sample_tube_or_well_id"][i])
            and not in_file["sample_tube_or_well_id"][i] == "-"
        ):
        ...
        if "all" in args.xml or "experiment" in args.xml:
            instrument = ""
            if (
                "instrument" not in in_file
                or pd.isna(in_file["instrument"][i])
                or in_file["instrument"][i] == "-"
            ):
            ...
            sample_ref = ""
            if (
                "biosample_accession" not in in_file
                or pd.isna(in_file["biosample_accession"][i])
                or in_file["biosample_accession"][i] == "-"
            ):
            ...
            library_selection = ""
            if (
                "library_selection" not in in_file
                or pd.isna(in_file["library_selection"][i])
                or in_file["library_selection"][i] == "-"
            ):
        ...
        if "all" in args.xml or "experiment" in args.xml or "runs" in args.xml:
            if (
                "library_strategy" not in in_file
                or pd.isna(in_file["library_strategy"][i])
                or in_file["library_strategy"][i] == "-"
            ):
            ...
            if library_strategy not in library_strategy_choice:
                exit(
                    library_strategy
                    + " is not one of the accepted values. Accepted values:"
                    + str(library_strategy_choice)
                )

            if (
                "read_type" not in in_file
                or pd.isna(in_file["read_type"][i])
                or in_file["read_type"][i] == "-"
            ):
            ...
            if read_type not in read_type_choice:
                if read_type == "HiC":
                    ...
                    exit(
                        read_type
                        + " is not one of the accepted values for read_type. Accepted values: "
                        + str(read_type_choice)
                    )

        aim = "assembly and annotation"
        if (
            "aim" in in_file
            and not pd.isna(in_file["aim"][i])
            and not in_file["aim"][i] == "-"
        ):
            aim = in_file["aim"][i]
        ...
        if read_type == library_strategy:
            ...
            experiments[rname] = (
                "exp_" + tolid_pref + "_" + library_strategy + "_" + sample_id
            )
        else:
            rname = (
                tolid_pref + "_" + read_type + "_" + library_strategy + "_" + sample_id
            )
            experiments[rname] = (
                "exp_"
                + tolid_pref
        ...
        forward_file_name = ""
        if (
            "forward_file_name" in in_file
            and not pd.isna(in_file["forward_file_name"][i])
            and not in_file["forward_file_name"][i] == "-"
        ):
            ...
            if (
                "forward_file_md5" not in in_file
                or pd.isna(in_file["forward_file_md5"][i])
                or in_file["forward_file_md5"][i] == "-"
            ):
        ...
        reverse_file_name = ""
        if (
            "reverse_file_name" in in_file
            and not pd.isna(in_file["reverse_file_name"][i])
            and not in_file["reverse_file_name"][i] == "-"
        ):
            ...
            if "fastq" in reverse_file_name:
                ...
                if (
                    "reverse_file_md5" not in in_file
                    or pd.isna(in_file["reverse_file_md5"][i])
                    or in_file["reverse_file_md5"][i] == "-"
                ):
        ...
        native_file_name = ""
        if (
            "native_file_name" in in_file
            and not pd.isna(in_file["native_file_name"][i])
            and not in_file["native_file_name"][i] == "-"
        ):
            native_file_name = in_file["native_file_name"][i]
            if (
                "native_file_md5" not in in_file
                or pd.isna(in_file["native_file_md5"][i])
                or in_file["native_file_md5"][i] == "-"
            ):
        ...
        library_attributes = ""
        if (
            "library_attributes" in in_file
            and not pd.isna(in_file["library_attributes"][i])
            and not in_file["library_attributes"][i] == "-"
        ):
        ...
        if (
            "experiment_attributes" in in_file
            and not pd.isna(in_file["experiment_attributes"][i])
            and not in_file["experiment_attributes"][i] == "-"
        ):
        ...
        if "all" in args.xml or "experiment" in args.xml:
            if (
                ...
                + "_"
                + sample_id
                not in experiment_register
                and "exp_" + tolid_pref + "_" + library_strategy + "_" + sample_id
                not in experiment_register
            ):
                experiment_register[
                    "exp_"
